...
def start_assi(user_id: str, 
    ...
    if not assigned_questions:
        ...
        if not questions:
            ...
            questions = generate_and_save_new_questions_services(prompt.text, prompt.id,lang, db)
...
def next_question(deck_id: int = Query(...), user_id: str = Query(...), db: Session = Depends(get_db_session)):
    ...
    if not available_assignments:
        ...
        if not unassigned_questions:
            ...
            available_assignments = generate_new_questions(user_id=user_id,prompt_id=deck.prompt_id,lang="en",db=db)
